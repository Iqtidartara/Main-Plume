import React from "react";

const Collections = () => {
  return (
    <section className="w-full">
      <main className="max-w-[80rem] mx-auto px-4 py-16">
        <div className="flex flex-col items-center justify-center gap-4">
          <h1 className="text-[#4A3D2E] text-[3.125rem] font-[700] leading-[100%] uppercase text-center">
            Our Winter collection
          </h1>
          <p className="text-[#4A3D2E] text-[1.2625rem] font-[400] leading-[140%] text-center">
            Discover the Cozy Chic: Our Winter Collection 2023. Stay <br/>warm while
            looking fabulous this season!
          </p>


        </div>
      </main>
    </section>
  );
};

export default Collections;
